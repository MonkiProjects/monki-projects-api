...
extension PlacemarkModel.Submission {
	...
	) -> EventLoopFuture<PlacemarkModel.Submission> {
		do {
			if isModerator {
	...
	private func reviewAsRegularUser(
		opinion: Review.Opinion,
		on database: Database
	) throws -> EventLoopFuture<PlacemarkModel.Submission> {
		switch self.state {
	...
	private func setNeedsChanges(on database: Database) -> EventLoopFuture<PlacemarkModel.Submission> {
