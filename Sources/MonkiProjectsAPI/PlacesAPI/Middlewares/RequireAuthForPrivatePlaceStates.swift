//
//  RequireAuthForPrivatePlaceStates.swift
//  PlacesAPI
//
...
internal struct RequireAuthForPrivatePlaceStates: Middleware {
	
	func respond(to request: Request, chainingTo next: Responder) -> EventLoopFuture<Response> {
		let authorized: [Place.State] = [.submitted, .published, .rejected]
		let state = try? request.query.get(Place.State.self, at: "state")
