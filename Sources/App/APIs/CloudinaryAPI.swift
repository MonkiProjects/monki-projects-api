...
let cloudinaryApi = CloudinaryAPIRoot()

struct CloudinaryAPIRoot: EndpointRoot {
	
	let scheme = "https"
	let host = "api.cloudinary.com"
	
	let cloudName = Environment.get("CLOUDINARY_CLOUD_NAME") ?? "nil"
	
	func uploadSateliteImage(from url: URL) -> Endpoint {
