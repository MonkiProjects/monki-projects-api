//
//  PlaceSubmissionControllerV1Tests.swift
//  PlacesAPITests
//
...
internal final class PlaceSubmissionControllerV1Tests: AppTestCase {
	...
	override func tearDownWithError() throws {
		...
		try self.place?.delete(force: true, on: app.db).wait()
	}
	
	// MARK: - Valid Domain
	
	/// Tests `POST /places/{placeId}/submit`.
	///
	/// - GIVEN:
	///     - A user
	///     - A place
	/// - WHEN:
	///     - The creator of the place submits it
	/// - THEN:
		...
		try app.test(
			...
			afterResponse: { res in
				try res.assertStatus(.created) {
					...
					XCTAssertEqual(submission.place, Self.placeId)
	...
	/// Tests `GET /places/{placeId}/submission`.
	///
	/// - GIVEN:
	///     - A submitted place
	/// - WHEN:
	...
	func testGetSubmissionReport() throws {
		...
		try app.test(
			.GET, "places/v1/\(Self.placeId)/submission",
			afterResponse: { res in
				try res.assertStatus(.ok) {
					let result = try res.content.decode(Submission.Public.self)
					
					XCTAssertEqual(result.id, submissionId)
					XCTAssertEqual(result.place, Self.placeId)
				}
			}
		)
	}
	
	/// Tests `POST /places/{placeId}/submission/reviews`.
	///
	/// - GIVEN:
	///     - A submitted place
	/// - WHEN:
	...
	func testAddSubmissionReview() throws {
		...
		try app.test(
			.POST, "places/v1/\(Self.placeId)/submission/reviews",
			beforeRequest: { req in
				let bearerAuth = BearerAuthorization(token: token.value)
				req.headers.bearerAuthorization = bearerAuth
	...
	func testReviewSubmissionNeedsChangesCreatesNewSubmission() throws {
		...
		let create = Review.Create(
			...
			comment: "This place needs changes",
			issues: [
				.init(
					reason: .coordinates,
					comment: "The place is not correcly located"
				),
			]
		)
	...
	/// Tests that adding enough reviews will publish a submitted place.
	///
	/// - GIVEN:
	///     - A submitted place
	///     - Enough users to publish the place
	/// - WHEN:
	///     - A user submits a review saying the submission needs changes
	/// - THEN:
	///     - `HTTP` status should be `200 OK`
	///     - The reviewed place should be published
	///
	/// # Notes: #
	...
	/// Tests that a user cannot submit someone else's place.
	///
	/// - GIVEN:
	///     - A place
	///     - A user that's not the creator of the place
	/// - WHEN:
	///     - The user tries to submit the place
	/// - THEN:
	...
	/// Tests that a user cannot submit their place twice.
	///
	/// - GIVEN:
	///     - A user
	///     - A user's place already submitted
	/// - WHEN:
	///     - The user tries to submit the place again
	/// - THEN:
	///     - `HTTP` status should be `403 Forbidden`
	...
	///     - A submitted place
	/// - WHEN:
	///     - The creator of the place tries to review their own submission
	/// - THEN:
	///     - `HTTP` status should be `403 Forbidden`
	...
	func testCreatorCannotReviewOwnPlace() throws {
		let app = try XCTUnwrap(Self.app)
		let token = try XCTUnwrap(Self.users[0].1)
		let (_, submissionId, _) = try submitPlace(on: app.db)
	...
	private func submitPlace(on database: Database) throws -> (PlaceModel, UUID, SubmissionModel) {
		...
		return (place, submissionId, submission)
	}
